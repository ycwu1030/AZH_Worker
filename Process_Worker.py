...
parser.add_argument('-pb', dest='paramfile_bkg',
                    default='DATADIR/param_bkg.json')
parser.add_argument('-s', dest='FLAG_SIG', action='store_true')
# Default calculate the total for signal, not components
parser.add_argument('-sc', dest='FLAG_SIG_COMPONENTS', action='store_true')
...
if FLAG_XEC:  # Calculate Cross Section, keep the value for future reference
    res2 = CALCULATE_CS(BKG_PROCS['TOTAL'], WORKDIR, DATADIR, {
                        'ID': 'bkg', 'PARAM': {}})
    print(res2)
    ...
    if FLAG_SIG:
        ...
        for PARAM_KEY in PARAMS.keys():
            ...
            for procid in SIG_TO_BE_CALCULATED.keys():
                if procid not in CS.keys():
                    CS[procid] = CALCULATE_CS(
                        SIG_TO_BE_CALCULATED[procid], WORKDIR, DATADIR, PARAM, YUKTYPE)
            if 'ROOT' not in PARAM.keys():
                PARAM['ROOT'] = {'3l': {}, '4l': {}}
            # , '4l': 'tmp_cards/madspin_card_dilepton.dat'}
            CHANLIST = {'3l': 'tmp_cards/madspin_card_semilep.dat'}
            for chan in CHANLIST.keys():
                ...
                for procid in SIG_TO_BE_CALCULATED.keys():
                    ...
                    for ntimes in range(START, NRUNS):
                        tmp = GENERATE_EVENTS(
                            SIG_TO_BE_CALCULATED[procid], WORKDIR, DATADIR, USEDPARAM, CARDS, YUKTYPE)
                        if procid not in ROOTLISTCUR.keys():
        ...
        for pid in BKG_PROCS.keys():
            for ntimes in range(NRUNS):
                CARDS['MADSPIN'] = join(
                    CURDIR, 'tmp_cards/madspin_card_semilep.dat')
                GENERATE_EVENTS(BKG_PROCS[pid], WORKDIR, DATADIR, {
                                'ID': 'bkg', 'CHAN': '3l', 'PARAM': {}}, CARDS)
...
if FLAG_ANA:
    ...
    if FLAG_SIG:
        ...
        for PARAM_KEY in PARAMS.keys():
            for procid in SIG_TO_BE_CALCULATED.keys():
                AZH_Pre_Analysis(
                    SIG_TO_BE_CALCULATED[procid], procid, DATADIR, PARAMS[PARAM_KEY], YUKTYPE)
    else:
        for PARAM_KEY in PARAMS.keys():
            AZH_Pre_Analysis(BKG_PROCS['TOTAL'],
                             'TOTAL', DATADIR, PARAMS[PARAM_KEY])
...
if FLAG_PLOT:
    ...
    for PARAM_KEY in PARAMS.keys():
        AZH_Plot(PARAMS[PARAM_KEY], PARAMS_BKG['bkg'],
                 SIG_TOTAL['TOTAL'], BKG_PROCS['TOTAL'], DATADIR, YUKTYPE)
...
if FLAG_NLL:
    ...
    with open(outputfile, 'w') as f:
        ...
        for PARAM_KEY in PARAMS.keys():
            ...
            NLL, MU = AZH_NLL(
                PARAM_SIG, PARAMS_BKG['bkg'], SIG_TOTAL['TOTAL'], BKG_PROCS['TOTAL'], DATADIR, YUKTYPE)
            # print(PARAM_SIG['PARAM']['MHA'],PARAM_SIG['PARAM']['MHH'],PARAM_SIG['PARAM']['tb'],PARAM_SIG['AUX']['WidthRatio'],NLL)
            f.write('%.3f %.3f %.3f %.3f %.3f %.6f %.6f %.6f\n' % (PARAM_SIG['PARAM']['MHA'], PARAM_SIG['PARAM']['MHH'], PARAM_SIG['PARAM'][
                    'tb'], PARAM_SIG['AUX']['HA_WidthRatio'], PARAM_SIG['AUX']['HH_WidthRatio'], PARAM_SIG['CS']['TOTAL'], float(MU), float(NLL)))
            f.flush()
