...
class MG_RUNNER(object):
    ...
    def __init__(self, MG5DIR, WORKDIR):
        self.MG5_DIR = MG5DIR
    ...
    def Generate_Process(self, PROC):
        ...
        subprocess.call('python %s/bin/mg5_aMC %s'%(self.MG5_DIR,tmpfile),shell=True)
    ...
    def Run_MadEvent(self,PROCNAME,DATADIR,PARAMS,CARDS = None,SQRTS = 14):
        ...
        CMD_FILE_NAME='%s_command_%s_%s_%s_%s.txt'%(log_prefix,PROCNAME,paramid,chanid,timetag)
        LOG_FILE_NAME='%s_output_%s_%s_%s_%s.txt'%(log_prefix,PROCNAME,paramid,chanid,timetag)
        ...
        # * Run the MadEvent
        subprocess.call('python %s/bin/madevent %s > %s'%(PROCDIR,CMDFILE,LOGFILE),shell=True)
        res=subprocess.check_output('awk \'$1=="Cross-section" {print $3}\' %s'%(LOGFILE),shell=True)
        ...
        root_file_name = 'N.A.'
        if CARDS:
            root_file_name = 'delphes_%s_%s_%s_%s.root'%(PROCNAME,paramid,chanid,timetag)
            copyfile(join(PROCDIR,'Events/%s_decayed_1/tag_1_delphes_events.root'%(PROCNAME)),join(PROC_DATA_DIR,root_file_name))
        subprocess.call('rm -rf %s'%(PROCDIR),shell=True)
